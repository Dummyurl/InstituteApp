...
public class GuestDataEnableAdapter extends RecyclerView.Adapter<GuestDataEnableAdapter.MyViewHolder> {
    Context context;
    ...
    public GuestDataEnableAdapter(Context context, RecyclerView recyclerView, List<Data> datas) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        try {
            ...
            try {
                ...
                PrintCatchException pc = new PrintCatchException(context,itemView, "GuestDataEnableAdapter", "onBindViewHolder", e);
                pc.showCatchException();
            }
            String ActiveStatus = datas.get(position).getAcFlagGuest().toString();

            if (ActiveStatus.equals("Y")) {
                holder.chkactiveuserCheck.setChecked(true);
                datas.get(position).setAcFlagGuest("Y");
            } else {
                holder.chkactiveuserCheck.setChecked(false);
                datas.get(position).setAcFlagGuest("N");
            }
            ...
            holder.chkactiveuserCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    ...
                    if (isChecked) {
                        datas.get(finalPostion).setAcFlagGuest("Y");
                    } else {
                        datas.get(finalPostion).setAcFlagGuest("N");
                    }
                }
            });
        } catch (Exception ex) {
    ...
    public void setFilter(List<Data> testnewlist) {
        try {
            ...
            PrintCatchException pc = new PrintCatchException(context,itemView, "GuestDataEnableAdapter", "setFilter", e);
            pc.showCatchException();
        }
    }
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        CustomTextView txtNameOfUser, txtAcDate,txtTextData, txtImageData, txtVideoData, txtPDFData;
        ...
        public void onClick(View view) {
            try {
                ...
                newIntent.putExtra("UploadAnswerSheet", "P");
    ...
    public void selectAll() {
        try {
            for (int i = 0; i < datas.size(); i++) {
                datas.get(i).setAcFlagGuest("Y");
                datas.get(i).setChecked("Y");
            }
            notifyDataSetChanged();
        } catch (Exception e) {
            PrintCatchException pc = new PrintCatchException(context,itemView, "GuestDataEnableAdapter", "selectAll", e);
            pc.showCatchException();
        }
    }
    ...
    public void deselectAll() {
        try {
            for (int i = 0; i < datas.size(); i++) {
                datas.get(i).setAcFlagGuest("N");
                datas.get(i).setChecked("N");
            }
            notifyDataSetChanged();
        } catch (Exception e) {
            PrintCatchException pc = new PrintCatchException(context,itemView, "GuestDataEnableAdapter", "deselectAll", e);
            pc.showCatchException();
        }
    }
